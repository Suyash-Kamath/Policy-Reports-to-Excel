...
async def process_file(
    ...
    try:
        ...
        2. Match the company name ({company_name}) with the formula grid
        3. Find the appropriate rule based on:
           - LOB segment
           - Company name (match with companies like DIGIT, BAJAJ, ICICI, SBI, etc.)
           - Policy type (TP, COMP, SAOD, etc.)
           - Payin category
        4. Apply the correct formula:
           - If rule says "90% of Payin" → multiply Payin_Value by 0.9
           - If rule says "88% of Payin" → multiply Payin_Value by 0.88
           - If rule says "-2%" → subtract 2 from Payin_Value
           - If rule says "-3%" → subtract 3 from Payin_Value
           - If rule says "-4%" → subtract 4 from Payin_Value
           - If rule says "-5%" → subtract 5 from Payin_Value
           - If rule says "Less 2 of Payin" → subtract 2 from Payin_Value
        5. Calculate: Calculated Payout = Apply the formula to Payin_Value
        6. Round down Calculated Payout to the nearest whole number
        7. Format as percentage (e.g., 19 → "19%")

        Use your intelligence to match segments with the grid rules even if naming varies slightly.
        For company matching, be smart about variations (e.g., "Bajaj Allianz" matches "Bajaj").

        Return JSON array with all original fields plus:
        - 'Calculated Payout' (as percentage string)
        - 'Formula Used' (the rule applied)
        - 'Rule Explanation' (brief explanation of how the rule was matched)
        """
