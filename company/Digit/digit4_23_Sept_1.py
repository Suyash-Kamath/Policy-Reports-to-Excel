...
# #         if not payin_str or payin_str.strip().upper() == "NIL":
# #             logger.warning(f"Invalid payin value: {payin_str}, defaulting to 0")
...
# #         logger.warning(f"Could not parse payin value: {payin_str}, defaulting to 0")
# #         return 0.0, "Payin Below 20%"
...
# #         # Handle "NIL" or invalid Payin values
# #         if not payin_str or payin_str.strip().upper() == "NIL":
# #             payin_value = 0.0
# #         else:
# #             try:
# #                 payin_value = float(payin_str.replace('%', '').replace(' ', '') or 0)
# #             except (ValueError, TypeError):
# #                 logger.warning(f"Invalid payin value: {payin_str}, defaulting to 0")
# #                 payin_value = 0.0
...
# #         # Create result record with "NIL" for empty or None values
# #         result_record = {
# #             'Location': location if location and location.strip() else "NIL",
# #             'Segment': segment if segment and segment.strip() else "NIL",
# #             'Policy': (segment_policy_type if segment_policy_type else policy_type) if (segment_policy_type or policy_type) and (segment_policy_type or policy_type).strip() else "NIL",
# #             'Payin': f"{int(payin_value)}%" if payin_value is not None and payin_value != 0 else "NIL",
# #             'Payout': f"{int(calculated_payout)}%" if calculated_payout is not None and calculated_payout != 0 else "NIL",
# #             'Remarks': remarks if remarks and remarks.strip() else "NIL",
# #             'Rule Explanation': rule_explanation if rule_explanation and rule_explanation.strip() else "NIL"
# #         }
...
# # - **Create a record for each non-NIL CD2 value:**
# #   - If 1+1 CD2 has a value (non-NIL), create a record with Policy="COMP" and Payin from 1+1 CD2
# #   - If SATP CD2 has a value (non-NIL), create a record with Policy="TP" and Payin from SATP CD2, appending "SATP" to Segment
# #   - If both 1+1 CD2 and SATP CD2 have values, create two records (one for COMP, one for TP)
# #   - If 1+1 CD2 is NIL but SATP CD2 has value, create only TP record using SATP value, Policy="TP", append "SATP" to Segment
# #   - If SATP CD2 is NIL but 1+1 CD2 has value, create only COMP record using 1+1 value, Policy="COMP"
# #   - If both are NIL or missing, create one record with Policy="COMP", Payin="0%", Remarks="Both CD2 values NIL"
...
# # - **Create records based on non-NIL CD2 values:**
# #   - 1+1 CD2 value → Policy="COMP", Payin=1+1 CD2
# #   - SATP CD2 value → Policy="TP", Payin=SATP CD2, Segment=Segment + " SATP"
# #   - Both values → Two records (COMP and TP)
# #   - 1+1 NIL and SATP has value → TP record only
# #   - SATP NIL and 1+1 has value → COMP record only
...
# # - "Segment": from Agency/PB segment field (include vehicle type details, append "SATP" for TP records)
# # - "Policy": "COMP" for 1+1 CD2 values, "TP" for SATP CD2 values
# # - "Payin": from CD2 values only (convert to percentage format, use "0%" if missing or both NIL)
# # - "Remarks": ALWAYS include additional information including vehicle makes (e.g. Hero/Honda), age info (e.g. Upto 180cc), validity, etc. If no extra info, set to "NIL". If one CD2 is NIL, note it in remarks like "SATP CD2 is NIL" or "1+1 CD2 is NIL"
...
# #     "Policy": "COMP",
# #     "Payin": "30%",
# #     "Remarks": "New Two Wheeler, SATP CD2 is NIL"
# #   }},
# #   {{
# #     "Location": "RJ_Good",
# #     "Segment": "TW SATP",
# #     "Policy": "TP",
# #     "Payin": "25%",
# #     "Remarks": "Third Party Only, 1+1 CD2 is NIL"
# #   }},
# #   {{
# #     "Location": "Example",
...
# # 3. **Create a COMP record for non-NIL 1+1 CD2 values**
# # 4. **Create a TP record for non-NIL SATP CD2 values, appending "SATP" to Segment**
# # 5. **If both 1+1 CD2 and SATP CD2 have values, create two records**
# # 6. **If 1+1 CD2 is NIL but SATP CD2 has value, create TP record only**
# # 7. **If SATP CD2 is NIL but 1+1 CD2 has value, create COMP record only**
# # 8. **If both 1+1 CD2 and SATP CD2 are NIL or missing, create one COMP record with Payin="0%"**
# # 9. **1+1 = COMP, SATP = TP for policy determination**
# # 10. **PVT Taxi goes under TAXI segment**
# # 11. **MISD includes Tractor, Cranes, Garbage Vans**
# # 12. **Parse every row in the table, do not skip any row, even if one or both values are NIL**
# # 13. **Always include all rows from the text, processing each one independently**
...
# #             model="gpt-4o",
# #             messages=[
# #                 {"role": "system", "content": "Extract insurance policy data as JSON array for Digit format. CRITICAL: Ignore CD1 completely, only use CD2. Create COMP for non-NIL 1+1 CD2, TP for non-NIL SATP CD2. Handle NIL symmetrically for both. If both NIL, create COMP with 0%. Parse all rows without skipping. Return ONLY valid JSON array."},
...
# #                 record['Payin_Value'] = 0.0  # Default value
...
# #         - **Smart NIL Handling**: If 1+1 CD2 (COMP) is NIL but SATP CD2 (TP) has value, uses SATP value and sets Policy=TP
...
# #         - **If 1+1 CD2 NIL but SATP CD2 has value → Use SATP, Policy=TP**
...
# #                 st.info("✅ NIL 1+1 CD2 with SATP CD2 value → Used SATP and set Policy=TP")
...
# #                 st.info("🔍 CD1 ignored, CD2 used. NIL COMP with TP value handled correctly.")
...
# # - "Agency/PB segment" = Segment (extract vehicle type, e.g., TW 1+5, PVT CAR, etc.)
# # - **CD2 = Payin percentage (COMPLETELY IGNORE CD1 - if CD1 columns exist, ignore them entirely)**
...
# #     "Policy": "COMP",
# #     "Payin": "61.5%",
# #     "Remarks": "Electric Vehicle, SATP CD2 is NIL"
# #   }},
# #   {{
# #     "Location": "RJ_Good",
# #     "Segment": "TW 1+5",
...
# #     "Policy": "COMP",
# #     "Payin": "0%",
# #     "Remarks": "Both 1+1 CD2 and SATP CD2 are NIL"
# #   }}
# # ]
...
# #             pdf_base64 = base64.b64encode(file_bytes).decode("utf-8")
# #             prompt = """Extract insurance policy details from this PDF.
# #             Focus on identifying Agency/PB Clusters (locations), segments (e.g., SC/EV, MC, TW 1+5), CD2 values (ignore CD1), and policy codes.
# #             If 1+1 CD2 is NIL but SATP CD2 has a value, note this relationship."""
# #             response = client.chat.completions.create(
# #                 model="gpt-4o",
# #                 messages=[{
...
# # def apply_formula_directly(policy_data, company_name):
# #     """
# #     Apply formula rules directly using Python logic based on segment, policy type, insurer, and payin range
# #     """
# #     calculated_data = []
...
# #         # Determine LOB from segment - Add 'MC', 'SC', 'EV' for TW mapping
# #         lob = ""
# #         if any(tw_keyword in segment for tw_keyword in ['TW', '2W', 'TWO WHEELER', 'BIKE', 'MC', 'SC', 'EV']):
# #             lob = "TW"
# #         elif any(car_keyword in segment for car_keyword in ['PVT CAR', 'PRIVATE CAR', 'CAR']):
# #             lob = "PVT CAR"
# #         elif any(cv_keyword in segment for cv_keyword in ['CV', 'COMMERCIAL', 'LCV', 'GVW', 'TN', 'PCV', 'GCV']):
...
# #         # Collect all matching rules and select the most specific one
# #         candidate_rules = []
# #         for rule in FORMULA_DATA:
# #             if rule["LOB"] != lob:
# #                 continue
...
# #             # Add to candidate rules
# #             candidate_rules.append(rule)

# #         # Select the most specific rule based on payin category
# #         for rule in candidate_rules:
# #             rule_remarks = rule.get("REMARKS", "").upper()
# #             if "PAYIN" in rule_remarks and rule_remarks == payin_category.upper():
# #                 matched_rule = rule
# #                 rule_explanation = f"Match: LOB={lob}, Segment={rule['SEGMENT']}, Insurer={rule['INSURER']}, Remarks={rule_remarks}"
# #                 break
# #         if not matched_rule:  # Fallback to NIL or default rule
# #             for rule in candidate_rules:
# #                 rule_remarks = rule.get("REMARKS", "").upper()
# #                 if rule_remarks == "NIL" or "NIL" in rule_remarks:
# #                     matched_rule = rule
# #                     rule_explanation = f"Match: LOB={lob}, Segment={rule['SEGMENT']}, Insurer={rule['INSURER']}, Remarks=NIL"
# #                     break
...
# #                     column_letter = chr(64 + col_idx) if col_idx <= 26 else str(col_idx)
# #                 max_length = max(max_length, len(str(column_name)))
# #                 for row_idx in range(4, len(df_calc) + 4):
# #                     cell_value = worksheet.cell(row=row_idx, column=col_idx).value
# #                     if cell_value:
# #                         max_length = max(max_length, len(str(cell_value)))
...
# # def main():
# #     st.set_page_config(page_title="Insurance Policy Processing - Digit Format", page_icon="📋", layout="wide")
# #     st.title("🏢 Insurance Policy Processing System - Digit Format")
# #     st.markdown("---")
# #     with st.sidebar:
# #         st.header("📁 File Upload")
# #         company_name = st.text_input("Company Name", value="Digit", help="Enter the insurance company name")
# #         policy_file = st.file_uploader(
# #             "📄 Upload Policy File",
# #             type=['pdf', 'png', 'jpg', 'jpeg', 'txt', 'csv', 'xlsx', 'xls', 'xlsm'],
# #             help="Upload your insurance policy document from Digit"
...
# #         📊 **Digit Format Mapping:**
# #         - Agency/PB Clusters → Location
# #         - Agency/PB segment → Segment (e.g., SC/EV, MC mapped to TW)
# #         - **CD2 → Payin (CD1 IGNORED)**
# #         - **1+1 CD2 NIL + SATP CD2 value → Use SATP, Policy=TP**
# #         - 1+5 → New TW COMP
# #         - 1+1 → Old COMP (if has value)
...
# #         - MISD includes Tractor, Cranes, Garbage Vans
# #         """)
# #         process_button = st.button("🚀 Process Digit Policy File", type="primary", disabled=not policy_file)
# #     if not policy_file:
# #         st.info("👆 Please upload a Digit policy file to begin processing.")
# #         st.markdown("""
# #         ### 📋 Digit Format Instructions:
...
# #           - SATP = TP (Third Party)
# #         - **Segment Types**:
# #           - TW: 1+5 (New), 1+1 (Old), SC/EV, MC (Motorcycle), includes bikes
# #           - PVT CAR: COMP + SAOD (+ means OR), TP
# #           - CV: GVW, PCV 3W, GCV 3W
# #           - BUS: School Bus, Staff Bus
# #           - TAXI: includes PVT Taxi
...
# #         - **If CD1 columns exist → IGNORE COMPLETELY**
# #         - **Only extract from CD2 columns**
# #         - **If 1+1 CD2 NIL but SATP CD2 has a value → Use SATP, Policy=TP**
# #         - **If both have values → Create 2 records (COMP + TP)**
# #         - **If both NIL → Create COMP record with Payin=0%**
...
# #         else:
# #             payin_category = "Payin Above 50%"
# #         # Handle "Payin Above 20%" as a fallback for specific insurers
# #         fallback_payin_category = "Payin Above 20%" if payin_value > 20 else None
...
# #         # Select the most specific rule based on payin category
# #         specific_ranges = ["Payin Below 20%", "Payin 21% to 30%", "Payin 31% to 50%", "Payin Above 50%"]
# #         for rule in candidate_rules:
# #             rule_remarks = rule.get("REMARKS", "").upper()
# #             if rule_remarks in specific_ranges and rule_remarks == payin_category.upper():
# #                 matched_rule = rule
# #                 rule_explanation = f"Match: LOB={lob}, Segment={rule['SEGMENT']}, Insurer={rule['INSURER']}, Remarks={rule_remarks}"
# #                 break
# #         if not matched_rule and fallback_payin_category:
# #             for rule in candidate_rules:
# #                 rule_remarks = rule.get("REMARKS", "").upper()
# #                 if rule_remarks == fallback_payin_category.upper():
# #                     matched_rule = rule
# #                     rule_explanation = f"Match: LOB={lob}, Segment={rule['SEGMENT']}, Insurer={rule['INSURER']}, Remarks={rule_remarks}"
# #                     break
# #         if not matched_rule:  # Fallback to NIL or default rule
...
#             insurers = [ins.strip().upper() for ins in rule["INSURER"].split(',')]
#             company_match = False
#             insurer_specificity = 1 if "ALL COMPANIES" in insurers or "REST OF COMPANIES" in insurers else 2
#             if "ALL COMPANIES" in insurers:
#                 company_match = True
#             elif "REST OF COMPANIES" in insurers:
#                 is_in_specific_list = False
...
#             candidate_rules.append((rule, insurer_specificity))

#         # Select the most specific rule based on payin category and insurer specificity
#         specific_ranges = ["Payin Below 20%", "Payin 21% to 30%", "Payin 31% to 50%", "Payin Above 50%"]
#         best_rule = None
#         best_specificity = -1
#         for rule, insurer_specificity in candidate_rules:
#             rule_remarks = rule.get("REMARKS", "").upper()
#             rule_specificity = insurer_specificity
#             if rule_remarks in specific_ranges or rule_remarks == "PAYIN ABOVE 20%":
#                 rule_specificity += 1  # Increase specificity for payin-based rules
#             if rule_remarks in specific_ranges and rule_remarks == payin_category.upper():
#                 if rule_specificity > best_specificity:
#                     best_rule = rule
#                     best_specificity = rule_specificity
#                     rule_explanation = f"Match: LOB={lob}, Segment={rule['SEGMENT']}, Insurer={rule['INSURER']}, Remarks={rule_remarks}"
#             elif rule_remarks == "PAYIN ABOVE 20%" and fallback_payin_category and fallback_payin_category.upper() == rule_remarks:
#                 if rule_specificity > best_specificity:
#                     best_rule = rule
#                     best_specificity = rule_specificity
#                     rule_explanation = f"Match: LOB={lob}, Segment={rule['SEGMENT']}, Insurer={rule['INSURER']}, Remarks={rule_remarks}"

#         if not best_rule:  # Fallback to NIL or default rule
#             for rule, insurer_specificity in candidate_rules:
#                 rule_remarks = rule.get("REMARKS", "").upper()
#                 if rule_remarks == "NIL" or "NIL" in rule_remarks:
#                     if insurer_specificity > best_specificity:
#                         best_rule = rule
#                         best_specificity = insurer_specificity
#                         rule_explanation = f"Match: LOB={lob}, Segment={rule['SEGMENT']}, Insurer={rule['INSURER']}, Remarks=NIL"
...
def apply_formula_directly(policy_data, company_name="Digit"):
    """
    Apply formula rules directly using Python logic based on segment, policy type, insurer, and payin range
    """
    calculated_data = []
    ...
    for record in policy_data:
        ...
        # Collect all matching rules with scoring for priority
        candidate_rules = []
        for rule in FORMULA_DATA:
            if rule["LOB"] != lob:
                continue
            ...
            # Check insurer match and calculate specificity
            insurers = [ins.strip().upper() for ins in rule["INSURER"].split(',')]
            company_match = False
            insurer_specificity = 0
            
            # Check if company matches specific insurers first
            specific_match = False
            for insurer in insurers:
                if insurer not in ["ALL COMPANIES", "REST OF COMPANIES"]:
                    if insurer in company_normalized or company_normalized in insurer:
                        specific_match = True
                        insurer_specificity = 3  # Highest priority for specific company match
                        company_match = True
                        break
            ...
            if not specific_match:
                if "ALL COMPANIES" in insurers:
                    company_match = True
                    insurer_specificity = 1  # Lower priority for "All Companies"
                elif "REST OF COMPANIES" in insurers:
                    # Check if company is NOT in any specific list for this LOB/Segment
                    is_in_specific_list = False
                    for other_rule in FORMULA_DATA:
                        if (other_rule["LOB"] == rule["LOB"] and 
                            other_rule["SEGMENT"] == rule["SEGMENT"] and
                    ...
                    if not is_in_specific_list:
                        company_match = True
                        insurer_specificity = 2  # Medium priority for "Rest of Companies"
            ...
            # Only consider payin-based rules if we have actual payin value > 0
            if payin_value > 0:
                if rule_remarks == payin_category.upper():
                    remarks_specificity = 5  # Highest for exact payin match
                elif rule_remarks == "PAYIN ABOVE 20%" and fallback_payin_category and fallback_payin_category.upper() == rule_remarks:
                    remarks_specificity = 4  # High for fallback payin match
                elif rule_remarks == "NIL":
                    remarks_specificity = 1  # Lowest for NIL when we have payin
            else:
                # For payin = 0, prioritize NIL rules
                if rule_remarks == "NIL":
                    remarks_specificity = 5  # High priority for NIL when payin is 0
                elif rule_remarks in ["PAYIN BELOW 20%", payin_category.upper()]:
                    remarks_specificity = 4  # Medium priority for payin rules

            total_score = insurer_specificity * 10 + remarks_specificity
            candidate_rules.append((rule, total_score, insurer_specificity, remarks_specificity))

        # Select the rule with highest total score
        if candidate_rules:
            candidate_rules.sort(key=lambda x: x[1], reverse=True)  # Sort by total score descending
            matched_rule = candidate_rules[0][0]
            rule_explanation = f"Match: LOB={lob}, Segment={matched_rule['SEGMENT']}, Insurer={matched_rule['INSURER']}, Remarks={matched_rule.get('REMARKS', 'NIL')}"

        # If no rule matched, use default
        if not matched_rule:
            # Default rule logic
            matched_rule = {"PO": "90% of Payin"}
            rule_explanation = f"No match for LOB={lob}, Segment={segment}, Insurer={company_name}, using default 90% of Payin"
