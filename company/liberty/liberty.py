...
#     """
#     Split a large text into smaller chunks to avoid context length errors.
#     """
#     chunks = []
#     start = 0
...
# def process_files(policy_file_bytes, policy_filename, policy_content_type, company_name):
#     """Main processing function with chunked OpenAI calls"""
#     try:
#         st.info("🔍 Extracting text from policy file...")
#         extracted_text = extract_text_from_file(policy_file_bytes, policy_filename, policy_content_type)
#         logger.info(f"Extracted text length: {len(extracted_text)}")
...
#         text_chunks = chunk_text(extracted_text, max_chars=15000)

#         for i, chunk in enumerate(text_chunks):
#             st.info(f"Processing chunk {i+1}/{len(text_chunks)}...")
#             parse_prompt = f"""
#             Analyze the following text, which contains insurance policy details.
#             Use your intelligence to extract data into JSON records with fields:
#             Segment, Location, Policy Type, Payin, Doable District, Remarks.
#             Text chunk:
#             {chunk}
#             """
#             response = client.chat.completions.create(
#                 model="gpt-4o-mini",
#                 messages=[
#                     {"role": "system", "content": "Extract policy data as JSON array. Return ONLY valid JSON array."},
#                     {"role": "user", "content": parse_prompt}
#                 ],
#                 temperature=0.1
#             )
...
#                 chunk_data = ensure_list_format(chunk_data)
#                 all_policy_data.extend(chunk_data)
#             except json.JSONDecodeError as e:
#                 logger.error(f"JSON decode error in chunk {i+1}: {str(e)}")
#                 continue

#         st.success(f"✅ Successfully parsed {len(all_policy_data)} policy records")
...
#         st.info("🧮 Applying formulas and calculating payouts...")
#         calculated_data = apply_formula_directly(all_policy_data, company_name)
#         st.success(f"✅ Successfully calculated {len(calculated_data)} records with payouts")

#         # Create Excel file as before
#         df_calc = pd.DataFrame(calculated_data)
#         output = BytesIO()
#         with pd.ExcelWriter(output, engine='openpyxl') as writer:
#             df_calc.to_excel(writer, sheet_name='Policy Data', startrow=2, index=False)
...
# Embedded Formula Data (same as before, assuming general rules apply to Liberty)
FORMULA_DATA = [
    {"LOB": "TW", "SEGMENT": "1+5", "INSURER": "All Companies", "PO": "90% of Payin", "REMARKS": "NIL"},
    {"LOB": "TW", "SEGMENT": "TW SAOD + COMP", "INSURER": "All Companies", "PO": "90% of Payin", "REMARKS": "NIL"},
    {"LOB": "TW", "SEGMENT": "TW SAOD + COMP", "INSURER": "DIGIT", "PO": "-2%", "REMARKS": "Payin Below 20%"},
...
def apply_formula_directly(policy_data, company_name):
    """
    Apply formula rules directly using Python logic for Liberty system
...
def process_files(policy_file_bytes, policy_filename, policy_content_type, company_name):
    """Main processing function with chunked OpenAI calls for Liberty company"""
    try:
        st.info("🔍 Extracting text from policy file...")
        extracted_text = extract_text_from_file(policy_file_bytes, policy_filename, policy_content_type)
        logger.info(f"Extracted text length: {len(extracted_text)}")
        ...
        for i, chunk in enumerate(text_chunks):
            st.info(f"Processing chunk {i+1}/{len(text_chunks)}...")
            parse_prompt = f"""
Analyze the following text from Liberty insurance Excel sheets. The text contains multiple sheets with payout grids for different segments like PC Comp, PCTP, TW Comp, CV COMP, etc.

Company Name: {company_name}

IMPORTANT: Always return a valid JSON array (list) of records, even if there's only one record. Create one JSON record per payout value in the rows, associating it with the geo segment, policy type, etc.

LIBERTY SPECIFIC MAPPING RULES:
1. Segment Analysis:
   - Sheets like "PC Comp_R_1", "PC Comp_R", "PC Comp": Segment "PVT CAR COMP + SAOD"
   - "PCTP_R_1", "PCTP_R", "PCTP": Segment "PVT CAR TP"
   - "TW Comp", "TW Comp_R", "TW Comp_R_1": Segment "TW SAOD + COMP" or "TW 1+5" based on type
   - "CV COMP_R_1_<7.5T", "CV COMP_R_1_>7.5T", "CV COMP_R", "CV Comp": Segment "CV" with sub-details like "Upto 2.5 GVW" or "All GVW"
   - "TW SATP_R_1", "TW SATP_R", "TW SATP": Segment "TW TP"
   - "CV SATP_R_1 (2)", "CV SATP_R_1", "CV SATP_R", "CV SATP": Segment "CV TP" or similar
   - Use "Sheet1" for mapping old geo segments to new, but focus ONLY on "Geo segment New" for Location field.

2. Extract into JSON records with these exact fields:
   - "Segment": Map from sheet name (e.g., "PVT CAR COMP + SAOD" for PC Comp sheets, "TW TP" for TW SATP)
   - "Location": Use ONLY "Geo segment New" column for location (e.g., "ANDHRA PRADESH - 1 KV"). IGNORE "Geo segment Old" column completely.
   - "Policy Type": Column header (e.g., "Comp - Petrol - NCB / NON NCB", "SATP Petrol <1000 cc", "New", ">1 - 5 Years")
   - "Payin": The payout value as percentage (e.g., "0.27999999999999997" → "28%")
   - "Doable District": "N/A" or any specific district if mentioned
   - "Remarks": Any additional notes, vehicle type details (e.g., "GCV 4W", age, CC range), or mappings

3. SPECIAL HANDLING:
   - For each row in payout sheets, create separate records for each payout column.
   - Use "Sheet1" to map old geo to new if needed, but only use "Geo segment New" for the Location field.
   - Handle different vehicle categories (e.g., Petrol, Diesel, CC ranges, GVW, age groups like "New", ">1 - 5 Years").
   - Include sub-details in Policy Type or Remarks (e.g., "Comp - Petrol - NCB", "SATP Petrol <1000 cc").
   - If value is 0 or empty, skip or mark as "N/A".

4. DATA STRUCTURE RECOGNITION:
   - Sheets have headers like "Geo segment New", "Geo segment Old", payout columns.
   - IGNORE "Geo segment Old" column completely; focus ONLY on "Geo segment New" for location information.
   - Mapping sheet "Sheet1": Use only for mapping old geo to new if necessary, but prioritize "Geo segment New".
   - Be intelligent in handling truncated text or variations.

If you cannot find specific information, use "N/A".
You are given raw text extracted from Liberty insurance Excel document.  
The document contains multiple sheets with payout grids.

### Your Task:
1. Detect all sheets and rows.
2. Convert each payout value into a JSON object.
3. Always output a single JSON array merging all data.
4. Normalize Payin to percentage format (e.g., "28%").
5. For Location, use ONLY the "Geo segment New" column and completely ignore the "Geo segment Old" column.

Note: If Remark contains: Zuno-21, it means calculated payout is 21% irrespective of payin.
...
"""
            response = client.chat.completions.create(
                model="gpt-4o-mini",
                messages=[
                    {"role": "system", "content": "Extract Liberty policy data as JSON array. Map sheet names to segments. Always rename payout fields to 'Payin'. Normalize Payin to percentage format. Extract ALL information for remarks. Return ONLY valid JSON array."},
                    {"role": "user", "content": parse_prompt}
                ],
                temperature=0.1
            )
...
def main():
    st.set_page_config(
        page_title="Insurance Policy Processing - Liberty", 
        page_icon="📋", 
        layout="wide"
    )
    
    st.title("🏢 Liberty Insurance Policy Processing System")
    st.markdown("---")
    ...
    with st.sidebar:
        ...
        # Company name input (fixed for Liberty, but allow override)
        company_name = st.text_input(
            "Company Name", 
            value="Liberty",
            help="Enter the insurance company name"
    ...
    if not policy_file:
        ...
        st.markdown("""
        ### 📋 Instructions:
        1. **Company Name**: Enter the insurance company name (default: Liberty)
        2. **Policy File**: Upload the document containing policy data (Excel preferred for Liberty grids)
        3. **Process**: Click the process button to extract data and calculate payouts
        
        ### 🎯 Features:
        - **Multi-format Support**: PDF, Images, Excel, CSV, Text files
        - **AI-Powered Extraction**: Uses GPT-4 for intelligent text extraction from Liberty sheets
        - **Enhanced Remarks Extraction**: Automatically detects vehicle types, age groups, CC/GVW ranges
        - **Smart Formula Application**: Uses embedded formula rules for accurate calculations
        - **Excel Export**: Download processed data as formatted Excel file
        ...
        try:
            ...
            with tab1:
                ...
                with col1:
                    st.metric("Total Records", len(results["calculated_data"]))
                with col2:
                    avg_payin = sum([r.get('Payin_Value', 0) for r in results["calculated_data"]]) / len(results["calculated_data"]) if len(results["calculated_data"]) > 0 else 0
                    st.metric("Avg Payin", f"{avg_payin:.1f}%")
                with col3:
                    segments = set([r.get('Segment', 'N/A') for r in results["calculated_data"]])
                    st.metric("Unique Segments", len(segments))
