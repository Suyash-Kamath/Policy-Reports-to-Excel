...
#         parse_prompt = f"""
#         Analyze the following text, which contains insurance policy grid details for SBI, in a tabular format. Use your intelligence to identify and extract the data accurately, treating it as a table with rows for regions/states/circles and columns for segments, sub-segments, vehicle ages, policy types, and payin values.

# Company Name: {company_name}

# IMPORTANT: Always return a valid JSON array (list) of records, even if there's only one record. Create one JSON record per payin value (each numerical cell in the table), associating it with the corresponding row headers (region, state name, circle) and column headers (segment, sub-segment, age, policy type).

# Extract into JSON records with these exact fields:
# - "Segment": Identify the main segment from column headers, e.g., "GCV 4W-Upto 2.5T GCV (except EV)", "12t to 20T (TATA & Ashok leyland)", "20t to 40T (TATA & Ashok leyland)". Include tonnage/weight/make details exactly as mentioned (e.g., "Upto 2.5T", "12t to 20T"). For sub-segments like makes (e.g., "Upto 2.0 T-All makes & 2.0-2.5T Tata makes", "2.0-2.5T-other than Tata makes"), append them to the segment, e.g., "GCV 4W-Upto 2.5T GCV (except EV) - Upto 2.0 T-All makes & 2.0-2.5T Tata makes". Recognize this as CV (Commercial Vehicle) or GCV (Goods Carrying Vehicle) LOB, and prefix with "CV - " or "GCV - " if appropriate based on context.
# - "Location": Combine region, state name, and circle, e.g., "Region: North 2; State: UTTAR PRADESH (Rest); Circle: Lucknow". Use "N/A" for missing parts.
...
# - "Doable District": Extract any RTO lists or district information from notes, e.g., "KA22/KA23/.../KA71" or "UP13/UP20/.../UP94". If specific to a location or segment (e.g., for "Upto 2.5T GCV" in RO Karnataka or UTTAR PRADESH (Rest)), associate accordingly; otherwise, "N/A".
# - "Remarks": Include vehicle age from headers (e.g., "Age: 1-5 Years", "Age: above 5 years"), vehicle makes (e.g., "Vehicle Makes: TATA, Ashok Leyland"), transaction types if mentioned (e.g., "New", "Comp" implying comprehensive), validity (e.g., "Effective from 21st Sep25 to 31st Oct25"), and any other notes (e.g., "Festival Dhamaka Grid; All Broker - Above 25 Lakhs; Note-For less than 25 lac Grid would be lower by 2% & for less than 1 L only IRDA applicable", "Above mentioned Club applicable from 21st Sep 25 to 31st Oct25", "For doable & declined RTOs & model list-Same as previous month guidelines"). Combine with semicolons, e.g., "Vehicle Makes: TATA, Ashok Leyland; Age: 1-5 Years; Validity: from 21st Sep25 to 31st Oct25; Note-For less than 25 lac Grid would be lower by 2% & for less than 1 L only IRDA applicable".

# ENHANCED EXTRACTION FOR SBI GRIDS:
# - Treat the text as a table: Rows start with Region/State Name/Circle, followed by values.
# - Columns represent segments (e.g., "GCV 4W-Upto 2.5T GCV (except EV)"), sub-segments (makes), ages (e.g., "Age 1-5 Years"), and policy types (New-Comp (Net)/Comp (Net)/SATP (Net)).
...
# - For CV/GCV: Always include weight/tonnage/make in segment, e.g., "GCV 4W-Upto 2.5T GCV (except EV) - Upto 2.0 T-All makes & 2.0-2.5T Tata makes".
# - Look for patterns: Segments in top headers, sub-makes/ages as sub-headers, policy types as innermost.
# - Extract global notes (e.g., RTO lists, applicability) and apply to relevant records or duplicate if needed (e.g., RTO for specific states/segments).
...
#         if any(tw_keyword in segment_upper for tw_keyword in ['TW', '2W', 'TWO WHEELER']):
#             lob = "TW"
#             if "NEW" in segment_upper:
#                 segment = "TW 1+5"
#         elif any(car_keyword in segment_upper for car_keyword in ['PVT CAR', 'PRIVATE CAR', 'CAR']):
#             lob = "PVT CAR"
#             if "PACKAGE" in segment_upper or "COMP" in policy_type:
...
#                         segment_match = True
#             elif lob == "BUS":
#                 if "SCHOOL" in rule_segment and "SCHOOL" in segment_upper:
#                     segment_match = True
#                 elif "STAFF" in rule_segment and "STAFF" in segment_upper:
#                     segment_match = True
#             elif lob == "PVT CAR":
#                 if "COMP + SAOD" in rule_segment and any(keyword in segment_upper for keyword in ["COMP", "COMPREHENSIVE", "PACKAGE", "SAOD"]):
#                     segment_match = True
#                 elif "TP" in rule_segment and "TP" in segment_upper:
#                     segment_match = True
#             elif lob == "TW":
#                 if "1+5" in rule_segment and any(keyword in segment_upper for keyword in ["1+5", "NEW"]):
#                     segment_match = True
#                 elif "TP" in rule_segment and "TP" in segment_upper:
#                     segment_match = True
#             else:
...
#             if "90% of Payin" in po_formula:
#                 if "ZUNO - 21" in remarks:
#                     calculated_payout = 21
#                     rule_explanation += "; Special for Zuno: fixed 21 regardless of payin"
#                 else:
#                     calculated_payout = payin_value * 0.9
#             elif "88% of Payin" in po_formula:
#                 calculated_payout = payin_value * 0.88
...
#         parse_prompt = f"""
#         Analyze the following text, which contains insurance policy grid details from an Excel sheet for Zuno, structured in rows and tables. Use your intelligence to identify and extract the data accurately, treating it as tabular data with multiple sections (e.g., main table with Product Line/Region/With NCB, and CV Business table with Doable State/segments).

# Company Name: {company_name}

# IMPORTANT: Always return a valid JSON array (list) of records, even if there's only one record. Create one JSON record per payin value (e.g., each "With NCB" value or numerical cell in CV Business table), associating it with the corresponding row details (e.g., Product Line as segment, Region/RTO/Zone as location).

# Extract into JSON records with these exact fields:
# - "Segment": Use "Product Line" as the main segment (e.g., "Pvt Car Package (New/Rollover/Renewal)", "TW NEW", "CV Business - 3W GCV"). Include any embedded policy type or details. For "TW NEW", map to "TW 1+5" (new two-wheelers). For CV Business columns, use the column name as segment (e.g., "3W GCV", "Upto 2.5 TP & Package (Tata, Maruti & Mahindra)"). Recognize LOBs like Pvt Car as "PVT CAR", TW as "TW", CV as "CV".
# - "Location": Use "Region/RTO/Zone" or "Doable State" (e.g., "North only Delhi NCR, Chandigarh, Mohali and Panchkula, UP", "Gujrat"). Combine with any RTO specifics. Use "Pan India" if mentioned. Use "N/A" for missing.
# - "Policy Type": Extract from Product Line or column (e.g., "Package" means "COMP", "SAOD", "TP Only", "New-Comp", "SATP"). If "Package" is mentioned under "Pvt Car Package", use "COMP + SAOD"; if "COMP" or "Comprehensive" is mentioned, use "COMP"; if "SAOD" is mentioned, use "SAOD"; if "TP & Package", use "TP/COMP"; if "TP Only", use "TP". Default to "COMP/TP" if unclear.
# - "Payin": Convert "With NCB" or numerical values to percentage format (e.g., "0.25" → "25%", "35" → "35%"). Ensure it's a string with "%".
# - "Doable District": Extract RTO lists or district/state info (e.g., "GJ-05" for Surat, or from notes like "As per system approved RTOs"). If specific to location/segment, associate; otherwise, "N/A".
# - "Remarks": Include "Logic" (e.g., "As per system approved RTOs"), "Remarks" column, any notes (e.g., "For OD 85% and above discount PO 19.5%", "Fleet approvals 20 to 55 GVW", "Staff Bus: On Individual and Corporate name with proper permit", "Bolero Camper is decline", "For wrong MMV sourced premium recovery will be done"). Also include vehicle makes (e.g., "Vehicle Makes: Tata, Maruti & Mahindra"), age info if present, transaction types (e.g., "New/Rollover/Renewal"), validity if mentioned. Combine with semicolons, e.g., "Logic: As per system approved RTOs; Note: For OD 85% and above discount PO 19.5%; Vehicle Makes: Tata, Maruti & Mahindra".

# ENHANCED EXTRACTION FOR ZUNO GRIDS:
# - Treat the text as Excel rows: First section has IMD Code, Product Line (segment/policy), Region/RTO/Zone (location), With NCB (payin), Logic/Remarks.
# - Second section ("CV Business"): Rows with Doable State (location), columns as segments/policy types (e.g., "3W GCV" with payin "35").
# - For each payin value, create a record linking to location and segment/policy.
# - Map "Package" to "COMP" or "1+1" in policy type/remarks; for "Pvt Car Package", specifically set "Policy Type" to "COMP + SAOD".
# - Special: If "Zuno-21" in remarks, note "Payout directly 21 regardless of payin".
# - Preserve exact details: e.g., exclusions like "except Surat (GJ-05)".
# - Extract global notes (e.g., fleet approvals, staff bus conditions) and apply to relevant records or duplicate.
# - Generalize: Column headers may embed segments/policy (e.g., "Upto 2.5 TP & Package"), so parse intelligently.

# Be intelligent in identifying even if format varies.
# If you cannot find specific information, use "N/A".
...
#         # Enhanced mapping based on ZUNO data structure
#         if any(tw_keyword in segment_upper for tw_keyword in ['TW', '2W', 'TWO WHEELER']):
#             lob = "TW"
...
#             else:
#                 # Default based on context - Package usually means COMP
#                 formula_segment = "PVT CAR COMP + SAOD"
...
#             else:
#                 formula_segment = "SCHOOL BUS"  # Default
...
#             if rule_segment != formula_segment.upper():
#                 continue
...
#                 # No payin category check needed - apply directly
#                 matched_rule = rule
#                 rule_explanation = f"Direct match: LOB={lob}, Segment={formula_segment}, Company={rule['INSURER']}, No payin category check (NIL remarks)"
#                 break
#             elif "ZUNO" in rule_remarks and "21" in rule_remarks:
#                 # Special ZUNO rule - direct payout of 21
#                 matched_rule = rule
#                 rule_explanation = f"ZUNO special rule: LOB={lob}, Segment={formula_segment}, Company={rule['INSURER']}, Direct 21% payout"
#                 break
...
#             # Special handling for ZUNO-21 rule
#             if "ZUNO" in matched_rule.get("REMARKS", "") and "21" in matched_rule.get("REMARKS", ""):
#                 calculated_payout = 21
#                 rule_explanation += "; Special ZUNO rule: fixed 21% regardless of payin"
#             elif "90% of Payin" in po_formula:
#                 calculated_payout = payin_value * 0.9
#             elif "88% of Payin" in po_formula:
#                 calculated_payout = payin_value * 0.88
#             elif "Less 2% of Payin" in po_formula:
...
#    - CV Business section: Doable State | 3W GCV | 3W PCV | TP & Package | Staff Bus | Tractor | (payin values in each column)
...
Note : If Remark contains : Zuno-21 , itsimply means that the calculated payout is 21% irrespective of payin value so dont see PO .
