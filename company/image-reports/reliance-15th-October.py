...
# ALWAYS return a valid JSON ARRAY (list) of objects, even if there's only one record
# Each object must have these EXACT field names:

# "Segment": Standardized LOB + policy type (e.g., "TW TP", "PVT CAR COMP + SAOD", "Upto 2.5 GVW")
# "Location": location/region information (use "N/A" if not found)
# "Policy Type": policy type details (use "COMP/TP" if not specified)
# "Payin": percentage value (convert decimals: 0.625 → 62.5%, or keep as is: 34%)
# "Doable District": district info (use "N/A" if not found)
...
# First, identify the LOB: TW (Two Wheeler), PVT CAR (Private Car), CV (Commercial Vehicle), BUS, TAXI, MISD (Miscellaneous)
# Then, determine policy type: TP (Third Party), COMP (Comprehensive), SAOD (Stand Alone Own Damage), etc.
# Use your intelligence to standardize the "Segment" to EXACTLY match one of these predefined values based on the text content, keywords, tonnage, or descriptions. Do not invent new segments; map to the closest standard one:

# TW-related:

# If mentions 1+5 year plan or long-term policy: "1+5"
# If SAOD + COMP, Comprehensive + Own Damage, or similar full coverage: "TW SAOD + COMP"
# If TP only, Third Party, or liability-only: "TW TP"


# PVT CAR-related:

# If COMP + SAOD, Comprehensive + Own Damage, or full coverage: "PVT CAR COMP + SAOD"
# If TP only or Third Party: "PVT CAR TP"


# CV-related (Commercial Vehicle):

# If upto 2.5 Tn, 2.5 GVW, or similar low tonnage/weight (e.g., "upto 2.5", "2.5 Tn"): "Upto 2.5 GVW" for the specified insurer or company name only else "All GVW & PCV 3W, GCV 3W"
...
# If School Bus or educational: "SCHOOL BUS"
# If Staff Bus, corporate, or other bus types: "STAFF BUS"


# TAXI-related: "TAXI"
# MISD-related (Miscellaneous, Tractor, etc.): "Misd, Tractor"


# If ambiguous, infer from context (e.g., tonnage for CV, vehicle type for others) and default to the most logical standard segment.
...
# For Remarks field - extract ALL additional info:
...
# # 3. For Segment field:
# #    - First, identify the LOB: TW (Two Wheeler), PVT CAR (Private Car or PCI), CV (Commercial Vehicle), BUS, TAXI, MISD (Miscellaneous)
# #    - Then, determine policy type: TP (Third Party), COMP (Comprehensive, or synonyms like package, 1st party, 1+1), SAOD (Stand Alone Own Damage), etc.
# #    - Use your intelligence to standardize the "Segment" to EXACTLY match one of these predefined values based on the text content, keywords, tonnage, or descriptions. Do not invent new segments; map to the closest standard one:
# #      - TW-related:
...
# ALWAYS return a valid JSON ARRAY (list) of objects, even if there's only one record or no data is found.
# Each object must have these EXACT field names:

# "Segment": Standardized LOB + policy type (e.g., "TW TP", "PVT CAR COMP + SAOD", "Upto 2.5 GVW"). If unsure, use "Unknown".
# "Location": location/region information (use "N/A" if not found).
# "Policy Type": policy type details (use "N/A" if not specified, or "COMP/TP" if mixed).
# "Payin": percentage value (convert decimals: 0.625 → 62.5%, or keep as is: 34%, use "0%" if not found).
# "Doable District": district info (use "N/A" if not found).
# "Remarks": additional info including vehicle makes, age, transaction type, validity, and fuel type (use "N/A" if not found).
...
# Use your intelligence to standardize the "Segment" to EXACTLY match one of these predefined values based on the text content, keywords, tonnage, or descriptions. If no match, use "Unknown":
...
# If ambiguous, infer from context (e.g., tonnage for CV, vehicle type for others) and default to "Unknown".
...
# If you see whole numbers like 34, add % to make 34%
# If you see percentages, keep them as is
# Use the value from the "PO" column, "Payin" column, or any column indicating payout/payin
# If not found, use "0%"
...
# If a field is not found or unclear, use "N/A" or "0%" as specified.
# Return ONLY the JSON array, no other text or explanations.
# Ensure the JSON is valid and parseable (e.g., [{{...}}, {...}]).
# Do not extract or include the "Discount" column or its values in any field. Ignore it completely.
# The "PO" column or similar may contain Payin values - prioritize that for "Payin".
# Ignore these columns completely: Discount, CD1, any column with "discount" or "cd1".
# If the text is incomplete or ambiguous, make a best effort to fill fields with "N/A" and include a note in Remarks.
# Special Notes:

# "SATP" means TP, so map "Private Car SATP" to "PVT CAR TP".
# Extract fuel type (diesel, petrol) if present in two-column values and include in Remarks.
# "MCY" means motorcycle; treat it as TW LOB (motorcycles or scooters).
# Also Sometimes in the photo , only Location like UP1 or any other is metioned along with the percentage
