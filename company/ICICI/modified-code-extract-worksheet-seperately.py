...
#     """Main processing function, handling sheets separately"""
#     try:
#         st.info("🔍 Extracting text from policy file...")
        
#         # Extract text, which now returns a dictionary for Excel files
...
#             parse_prompt = f"""
#             Analyze the following text, which contains insurance policy details for sheet {sheet_name}.
...
#             Note: If the Payrate word is mentioned it means Payin , also if the PO is mentioned , it means Payin
#             Note: If possible , please mention the Segment which is mentioned in the text in the remarks only
...
#             # Create Excel file for this sheet
#             st.info(f"📊 Creating Excel file for {sheet_name}...")
...
#             results["excel_data"][sheet_name] = excel_data
...
#                 for sheet_name, excel_data in results["excel_data"].items():
#                     st.write(f"### {sheet_name}")
#                     st.download_button(
#                         label=f"📊 Download Excel File ({sheet_name})",
#                         data=excel_data,
#                         file_name=f"{company_name}_processed_policies_{sheet_name}.xlsx",
#                         mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
#                     )
#                     json_data = json.dumps(results["calculated_data"][sheet_name], indent=2)
#                     st.download_button(
...
#                         mime="text/csv"
#                     )
#                 st.info("💡 Each Excel file contains formatted data for a specific sheet with company header and calculated payouts.")
